...
gtFile =  open("GT_adm_039_150_430.txt")
count = 0;
for line in gtFile:
	w = line.split()
...
################## READ ANGLES ####################
angFile =  open("angles_adm_039_150_430.txt")
for line in angFile:
	w = line.split()
		...
		for m in range(len(Maps)):
			...
			for t in range(LenTraj):
				...
				for x in range(numTests):
					...
					dirZ = (-testZ[m][c][x][t]*62/0.330769231)/(0.188679245) #escala para metros:(-testZ[m][c][x][t]*62/0.330769231)	metros para px (-testZ[m][c][x][t]*62/0.330769231)/(0.188679245)			
					xCorr = testX[m][c][x][t];
					yCorr = testY[m][c][x][t];
...
if printCorrigido == 1:
	for c in range(len(Configs)):
		for m in range(len(Maps)):					
			plot(errorXYZcorr[m][c],color=cstyleCorr[(c%len(cstyle))],linestyle=lstyleCorr[(c%len(lstyle))],linewidth=2)
...
if len(ConfigsName) == 1:
	savefig("Graficos/" + MapsName[m] + "_" + ConfigsName[0] + "novo_z")
elif len(ConfigsName) == 3:
	savefig("Graficos/" + MapsName[m] + "_" + ConfigsName[1] + "_VS_" + ConfigsName[2] + "novo_z")#config 0 é o abBRIEF, estou colocand como referência
else:
	savefig("Graficos/" + MapsName[m] + "novo_z")
show()
