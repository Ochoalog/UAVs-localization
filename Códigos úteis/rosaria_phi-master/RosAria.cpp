...
#include "rosaria_phi/BumperState.h"
#include "tf/tf.h"
#include "tf/transform_listener.h"  //for tf::getPrefixParam
#include <tf/transform_broadcaster.h>
#include "tf/transform_datatypes.h"
#include <dynamic_reconfigure/server.h>
#include <rosaria_phi/RosAriaConfig.h>
...
class RosAriaNode
{
  ...
  public:
    ...
    void publish();
    void sonarConnectCb();
    void dynamic_reconfigureCB(rosaria_phi::RosAriaConfig &config, uint32_t level);
    void readParameters();

  protected:
    ros::NodeHandle n;
    ros::Publisher pose_pub;
    ros::Publisher absTruePose_pub;
    ros::Publisher relTruePose_pub;
    ...
    rosaria_phi::BumperState bumpers;
    ArPose pos;
    ...
    // dynamic_reconfigure
    dynamic_reconfigure::Server<rosaria_phi::RosAriaConfig> *dynamic_reconfigure_server;
...
void RosAriaNode::dynamic_reconfigureCB(rosaria_phi::RosAriaConfig &config, uint32_t level)
{
  //
  // Odometry Settings
  //
  ...
  publish_aria_lasers(false)
{
  ...
  absTruePose_pub = n.advertise<nav_msgs::Odometry>("absTruePose",1000);
  relTruePose_pub = n.advertise<nav_msgs::Odometry>("relTruePose",1000);

  bumpers_pub = n.advertise<rosaria_phi::BumperState>("bumper_state",1000);
...
int RosAriaNode::Setup()
{
  ...
  size_t colon_pos = serial_port.find(":");
    ROS_INFO_NAMED("rosaria", "serial_port: %s", serial_port.c_str());
...
//  argparser->loadDefaultArguments(); // adds any arguments given in /etc/Aria.args.  Useful on robots with unusual serial port or baud rate (e.g. pioneer lx)
  ...
  dynamic_reconfigure_server = new dynamic_reconfigure::Server<rosaria_phi::RosAriaConfig>;
  
  // Setup Parameter Minimums and maximums
  rosaria_phi::RosAriaConfig dynConf_min;
  rosaria_phi::RosAriaConfig dynConf_max;
  ...
  rosaria_phi::RosAriaConfig dynConf_default;
...
//    if (!laserConnector->connectLasers())
    {
      ROS_INFO_NAMED("rosaria", "rosaria: Error connecting to laser(s)...");
      return 1;
    }
...
//        tfname += ln; 
      ...
      laserPublisher = new LaserPublisher(l, n, true, tfname, "sick_lms100");
//    }
//    robot->unlock();
...
bool RosAriaNode::simStatPacketHandler(ArRobotPacket* packet) {
...
const Pose& RosAriaNode::getAbsoluteTruePose()
{
...
const Pose& RosAriaNode::getRelativeTruePose()
{
...
std::cerr << "AQUI!!!!" << std::cerr;
  ...
  if(isSimulation_){
      ...
      tf::poseTFToMsg(tf::Transform(tf::createQuaternionFromYaw(simAbsoluteTruePose_.theta*M_PI/180), tf::Vector3(simAbsoluteTruePose_.x, simAbsoluteTruePose_.y, 0)), position.pose.pose); //Aria returns pose in mm.
      position.header.frame_id = frame_id_odom;
      position.child_frame_id = frame_id_base_link;
      position.header.stamp = ros::Time::now();
      absTruePose_pub.publish(position);

      Pose simRelativeTruePose_ = getRelativeTruePose();

      tf::poseTFToMsg(tf::Transform(tf::createQuaternionFromYaw(simRelativeTruePose_.theta*M_PI/180), tf::Vector3(simRelativeTruePose_.x, simRelativeTruePose_.y, 0)), position.pose.pose); //Aria returns pose in mm.
      position.header.frame_id = frame_id_odom;
      position.child_frame_id = frame_id_base_link;
      position.header.stamp = ros::Time::now();
