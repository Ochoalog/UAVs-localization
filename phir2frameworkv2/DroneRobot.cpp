...
DroneRobot::DroneRobot(string& mapPath, string& trajectoryName, vector< heuristicType* > &heuristicTypes, bool quiet, string& oName, int startVal, int finishVal)
{
    ...
    // Read fullMap
    //cv::Mat originalMap = cv::imread(mapPath,CV_LOAD_IMAGE_COLOR);
    originalMap = cv::imread(mapPath,CV_LOAD_IMAGE_COLOR);

    if(! originalMap.data )                              // Check for invalid input
    {
        cout<<"Endereco:"<<mapPath<<endl;
        return;
    }
    isBRIEF = false; //ESTA FLAG ESTA SENDO USADA PARA GARANTIR QUE A QUANTIZACAO SO SEJA FEITA PARA O BRIEF
                     //NA FUNCAO 'run'
    if(heuristicTypes[0]->strategy == BRIEF){
        isBRIEF = true;
        cout<<"VOU FAZER QUANTIZACAO PORQUE E BRIEF - CONSTRUTOR"<<endl;
    ...
    //cvtColor(originalMap, originalMap, CV_BGR2Lab);
...
//    globalMaps.push_back(map);
    ...
    if(!anglesFile.is_open())    //Se não tiver esse arquivo, executará da forma original, sem roll e yaw.
    ...
    for(int i=0;i<heuristicTypes.size();++i)
    {
        ...
        {
            heur = new MutualInfHeuristic(MUTUAL_INFORMATION, id, hT->colorDifference, hT->threshold);
        }
...
/**
 * @brief DroneRobot::initialize
 *  Método responsável por inicializar o arquivo de log e instanciar o MCL.
 * @param numParticles
 */
/*void DroneRobot::initialize(int numParticles)
{
    cv::namedWindow( "Local Map", cv::WINDOW_KEEPRATIO );
    string oName;
    ...
    mcLocalization = new MCL(heuristics, cachedMaps, globalMaps, realPose, oName, numParticles);
...
{    
    ...
    if(isBRIEF)
    {
        cout<<"VOU FAZER QUANTIZACAO PORQUE E BRIEF - RUN"<<endl;

        if(mapPreProc == 1){
            currentMap = quantization(currentMap, numQuantization);
            //cv::Mat mappp  = quantization(currentMap, numQuantization);
            //cv::imshow( "ss", currentMap);
            //cv::imwrite("/home/phi/Documents/datasets/FOTOS-ADM-SEQUOIA-18_ABRIL/Imagens/0040/quantizadas/"  + std::to_string(step) + ".png", mappp);
        }
        ...
        else if(mapPreProc == 2)
        {
            ...
            bilateralFilter(currentMap, filteredLocalMap, 36, 300, 240, cv::BORDER_DEFAULT);
    ...
    //cv::cvtColor(currentMap, currentMap, CV_BGR2Lab);
...
//    resize(currentMap,window,cv::Size(0,0),0.2,0.2);
    ...
    if(!runQuiet)
    {
        ...
        {
            /*cv::Mat red;
            cv::resize(currentREDMap, red, cv::Size(), 0.25, 0.25);
            imshow( "Local Map RED", red );
            cv::Mat nir;
            cv::resize(currentNIRMap, nir, cv::Size(), 0.25, 0.25);
            imshow( "Local Map NIR", nir );
            cv::Mat gre;
            cv::resize(currentGREMap, gre, cv::Size(), 0.25, 0.25);
            imshow( "Local Map GRE", gre );
            cv::Mat reg;
            cv::resize(currentREGMap, reg, cv::Size(), 0.25, 0.25);
            imshow( "Local Map REG", reg );*/
            //TESTE====================
//            VegetationIndex *vegIdx = VegetatonIndexGen::InitVegetationIndex();

//            vegIdx->redVisibleMat = currentREDMap;
//            vegIdx->redEdgeMat = currentREGMap;
//            vegIdx->shortWaveMat = currentNIRMap;

//            cv::Size s = vegIdx->redVisibleMat.size();
//            vegIdx->width = s.width;
//            vegIdx->height = s.height;
//            int size = sizeof(unsigned char)*vegIdx->width*vegIdx->height*3;
//            unsigned char* result;
//            result = (unsigned char*)malloc(size);

            //O NDVI é calculado em cima das imagens com a radiance definida
//            vegIdx->setIndexSaveImg(currentNIRMap.data, currentREDMap.data, currentREGMap.data, currentGREMap.data, result);
//            cv::Mat imageResult;
//            imageResult.create(vegIdx->height,vegIdx->width,CV_8UC3);
//            imageResult.data = result;

            //Usar na odometria
//            currentNDVIMap = imageResult;
//            imwrite("/home/phi/Documents/Datasets/FOTOS-ADM-SEQUOIA-18_ABRIL/Imagens/0035/NDVI/" + std::to_string(step) + "_ndvi.png", imageResult);
            //imwrite("/home/phi/Documents/Datasets/Sequoia_Matrice100_05_02_2019/voo_100m/Filtradas/" + std::to_string(step) + "_red.png", currentREDMap);
            //imwrite("/home/phi/Documents/Datasets/Sequoia_Matrice100_05_02_2019/voo_100m/Filtradas/" + std::to_string(step) + "_nir.png", currentNIRMap);
            //imwrite("/home/phi/Documents/Datasets/Sequoia_Matrice100_05_02_2019/voo_100m/Filtradas/" + std::to_string(step) + "_reg.png", currentREGMap);
            //imwrite("/home/phi/Documents/Datasets/Sequoia_Matrice100_05_02_2019/voo_100m/Filtradas/" + std::to_string(step) + "_gre.png", currentREDMap);
//            cv::Mat ndvi;
//            cv::resize(imageResult, ndvi, cv::Size(), 0.25, 0.25);
//            imshow( "Local Map NDVI", ndvi );
            //========================
        }
         cv::waitKey(100);
    }
...
//    if(availableGTruth){
//        //ESTAVA DANDO ERRO #################################################
////        mcLocalization->writeErrorLogFile3d(realPose.x,realPose.y, realPose.z, realPose.yaw);
//        mcLocalization->writeErrorLogFile3d(realPose.x,realPose.y, realPose.z);
//    }
//    else{
//        cout<<"NO GROUND_TRUTH"<<endl;
...
{
   //if(anglesFile.peek() == fstream::traits_type::eof()) // keep the last good pose
   //    return realPose;
...
//Detecção de bordas utilizando Sobel
...
pair<Pose3d,bool> DroneRobot::findOdometryUsingECC(cv::Mat& prevImage, cv::Mat& curImage)
{
...
//    imshow("Image 2", im2_gray);
    ...
    catch(cv::Exception e)
    {
        if (e.code == cv::Error::StsNoConv)
        {
            ...
            return pair<Pose3d,bool>(odometry_,false);
        }
    }
...
//    imshow("Aligned", templ);
    ...
    p.yaw = acos(warp_matrix.at<float>(0,0));

    //@@@@@@@@@@@@@@@
    p.z = 1;

    return pair<Pose3d,bool>(p,true);
}
...
//    while(odom.second==false && cT<0.06){
    ...
    //Detectar linhas utilizando Sobel
    if(USE_SOBEL)
    {
    ...
    if(ODOM_TYPE >= 1 && ODOM_TYPE <= 3)
        ...
        odom = findOdometryUsingECC(prevIm, curIm);
    else if(ODOM_TYPE == 7)
        odom = findOdometryUsingTemplateMatching(prevIm, curIm);
...
pair<Pose3d,bool> DroneRobot::findOdometryUsingTemplateMatching(cv::Mat& prevImage, cv::Mat& curImage)
{
    bool isGood = false;
    Pose3d pose;
    cv::Mat whiteMask(curImage.rows, curImage.cols, CV_8UC3, cv::Scalar(255,255,255));
    ...
    while(ang <= 360)
    {
        templ = Utils::rotateImage(curImage, ang);
        mask = Utils::rotateImage(whiteMask, ang);
        imshow( "Current Image", templ );
        imshow( "Mask", mask );

        /// Source image to display
        cv::Mat img_display, result;
        prevImage.copyTo( img_display );

        cv::Point matchLoc = Utils::templateMatching(prevImage,templ,result,CV_TM_CCORR_NORMED,mask);

        /// Show me what you got
        rectangle( img_display, matchLoc, cv::Point( matchLoc.x + curImage.cols , matchLoc.y + curImage.rows ), cv::Scalar::all(0), 2, 8, 0 );
        rectangle( result, matchLoc, cv::Point( matchLoc.x + curImage.cols , matchLoc.y + curImage.rows ), cv::Scalar::all(0), 2, 8, 0 );
    ...
    pose.z = 1;
    return pair<Pose3d,bool>(pose, isGood);
}
...
//        printf( "-- Good Match [%d] Keypoint 1: %d  -- Keypoint 2: %d  \n", i, good_matches[i].queryIdx, good_matches[i].trainIdx );
    ...
    obj_corners[7] = cvPoint( img_1.cols/2 + img_1.cols/4, img_1.rows/2 + img_1.rows/4 ); // POINT TO CALCULATE DE Z COORDINATE
    obj_corners[8] = cvPoint( img_1.cols/2 - img_1.cols/4, img_1.rows/2 - img_1.rows/4 ); // POINT TO CALCULATE DE Z COORDINATE
...
//    if(good_matches.size()>=minNumPoints)
    if(!H.empty()){
        ...
        //Round scale to eliminate noise
        if(p.z < 1.01 && p.z > 0.99) p.z = 1;


        //float teste = atan2(obj_corners[5].y-obj_corners[4].y,obj_corners[5].x-obj_corners[4].x);//ângulo de 0 graus
        p.yaw = atan2(scene_corners[5].y-scene_corners[4].y,scene_corners[5].x-scene_corners[4].x);
    //    p.theta += M_PI/2.0;
        isGood = true;

        /*  Parâmetros intrínseocs padrão, considerando uma câmera sem nenhum distorção
         *  Camera.fx: 320 width/2
            Camera.fy: 320 width/2
            Camera.cx: 320 width/2
            Camera.cy: 240 height/2
        ...
        */
        /*cv::Mat1f K = (cv::Mat1f(3, 3) <<
                        ...
                        0, f, h/2,
                        0, 0,   1);
        int ret = decomposeHomographyMat(H, K);*/
...
pair<Pose3d,bool> DroneRobot::findOdometryUsingFeaturesSift(cv::Mat& prevImage, cv::Mat& curImage, double cT)
{
    ...
    cv::Ptr<cv::Feature2D> detector=cv::xfeatures2d::SIFT::create();         //create(20000,8,cT);
    cv::Ptr<cv::Feature2D> extractor=cv::xfeatures2d::SIFT::create();        //create(20000,8,cT);
