...
double UnscentedColorHeuristic::calculateValue(int x, int y, Pose3d p, cv::Mat *image, cv::Mat *map)
{
    ...
    for(int i = 0; i < points.size(); i++){
         firstPoint = cv::Point(((points[i].x-xPad)*scale*cos(theta)+(points[i].y-yPad)*scale*sin(theta))+p.x,
                                         ((points[i].x-xPad)*scale*-sin(theta)+(points[i].y-yPad)*scale*cos(theta))+p.y);
