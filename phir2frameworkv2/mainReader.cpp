...
vector<double> getGPSRelTimeNew(const vector<GPS>& gps)
{
    vector<double> gpsTimesInSec;
    for(unsigned int i=0; i<gps.size(); ++i)
        gpsTimesInSec.push_back((double)i);
...
vector<double> getIMURelTimeNew(const vector<IMU>& imu)
{
    vector<double> imuTimesInSec;
    for(unsigned int i=0; i<imu.size(); ++i)
        imuTimesInSec.push_back((double)i);
...
vector<double> getATTRelTimeNew(const vector<ATT>& att)
{
    vector<double> attTimesInSec;
    for(unsigned int i=0; i<att.size(); ++i)
        attTimesInSec.push_back((double)i);
...
void exportGoodTraj(vector<bool>& goodOnes, string tp, vector<cv::Point2i>& imagesPixelCoords, vector<double>& imagesHeadings, vector<double>& altitudes)
{
    // Read images names
    fstream input;
    ...
    while(input.peek() != fstream::traits_type::eof()){
        ...
        if(goodOnes[i]){
            ...
            gTruth << imagesPixelCoords[i].x << ' ' << imagesPixelCoords[i].y << ' ' << altitudes[i] << ' ' << imagesHeadings[i] << endl;
        }
...
void readLogFile(string lf, vector<IMU>& imu1, vector<IMU>& imu2, vector<GPS>& gps, vector<ATT>& att)
{
    ...
    logFile.Read();  //HERE ALL THE FILE ARE READ
...
void exportGroundTruth(string& lf, vector<cv::Point2i>& imagesPixelCoords, vector<double>& imagesHeadings)
{
    string rawname = lf.substr(0, lf.find_last_of("."));
    fstream gTruth;
...
int main(int argc, char* argv[])
{
    ...
    int qtdImagesDataset = 322; //DEPENDE DO DATASET
    ...
    lf = "/home/phi/Documents/datasets/FOTOS-ADM-SEQUOIA-18_ABRIL/Imagens/0040/Banco/traj_extracted.txt";
    tp = "/home/phi/Documents/datasets/FOTOS-ADM-SEQUOIA-18_ABRIL/Imagens/0040/Banco/times.txt";
    ...
    //HERE ALL VALUES ARE READ
    readLogFile(lf,imu1,imu2,gps,att);
    cout << "GPS " << gps.size() << " IMU1 " << imu1.size() << " IMU2 " << imu2.size() << endl;

    cout << "Antes de efetuar a transformação" << endl;
    // Initialize transform
    PixelTransform tf;
    cout << "Efetuou a transformação" << endl;
    vector<cv::Point2i> gpsPixelCoords = convertGPSToPixels(gps,tf);

    vector<double> gpsTimes;
    vector<double> attTimes;
    vector<double> imu1Times;
    vector<double> imu2Times;
    vector<double> imagesTimes;
    vector<double> altitudes;

    vector<bool> goodOnes;
    for(int x=0;x<qtdImagesDataset;x++){
        gpsTimes.push_back(x);
        attTimes.push_back(x);
        imu1Times.push_back(x);
        imu2Times.push_back(x);
        imagesTimes.push_back(x);
        goodOnes.push_back(true);

        altitudes.push_back(gps[x].values[7]);
    }
    ...
    exportGoodTraj(goodOnes, tp, imagesPixelCoords, imagesHeadings, altitudes);
...
//        line( img, imagesPixelCoords[l], imagesPixelCoords[l+1], Scalar( 255, 255, 0), 5);
//    }
...
//    plotIMU(imagesIMU,canvas);
    ...
    exportGroundTruth(lf, imagesPixelCoords, imagesHeadings);

    //estimateOdometry(imu1); não estou usando por hora
