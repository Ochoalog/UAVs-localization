...
double HistogramHeuristic::setObservedHistogram(int x, int y, cv::Mat *frame, cv::Mat *frameMap)
{
    int xini = x-radius;
    int yini = y-radius;
    int xend = x+radius;
...
double HistogramHeuristic::calculateValue(int x, int y, cv::Mat *image, cv::Mat *map)
{
    int xini = x-radius;
    int yini = y-radius;
    int xend = x+radius;
...
double MIHeuristic::calculateValue(int x, int y, cv::Mat &image, cv::Mat *map, cv::Mat *frame, cv::Mat *frameMap, Pose3d p)
{
...
//    cv::Mat subImg = Utils::getRotatedROIFromImage(p,cv::Point2f(size, size), image);
    ...
    cv::Range cRange(xini, xend+1);
    cv::Range rRange(yini, yend+1);
    cv::Mat subImg(image, rRange, cRange);
    cv::Point2f pt( subImg.cols/2.0f, subImg.rows/2.0f);
    cv::Mat r = cv::getRotationMatrix2D(pt, RAD2DEG(p.yaw)+90.0, 1.0);
